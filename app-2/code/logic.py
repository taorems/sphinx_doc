...
def calculate_factors(config_bin_settings: dict) -> np.ndarray:
    ...
    .. math::
        Total damage  = \sum_{i=1}^{n} [{factor_i} * {d_i} * {L_i}^m] = 1
        
    
    where :math:`factor_i` is the factor, :math:`d_i` is the design distance and :math:`L_i` is the design pinion load for each bin, and :math:`m` is the power. Since, design values are used, the total damage is equated to 1 and the factors are then calculated as follows:
